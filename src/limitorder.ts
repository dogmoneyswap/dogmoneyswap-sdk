...
export class LimitOrder {
    ...
    static getLimitOrder(data: ILimitOrderData): LimitOrder {
        return new LimitOrder(
            data.maker,
            CurrencyAmount.fromRawAmount(new Token(data.chainId, data.tokenIn, data.tokenInDecimals, data.tokenInSymbol), data.amountIn),
            CurrencyAmount.fromRawAmount(new Token(data.chainId, data.tokenOut, data.tokenOutDecimals, data.tokenOutSymbol), data.amountOut),
            data.recipient,
            data.startTime,
            data.endTime,
            data.stopPrice,
    ...
    signdOrderWithPrivatekey(chainId: ChainId, privateKey: string) {
        let order : Message = {
            ...
            oracleData: ethers.utils.keccak256(this.oracleData)
        }
    ...
    async signOrderWithProvider(chainId: ChainId, provider: ethers.providers.Web3Provider) {
        let order: Message = {
            maker: this.maker,
            tokenIn: this.tokenInAddress,
            tokenOut: this.tokenOutAddress,
...
export class FillLimitOrder {
    ...
    constructor(order: LimitOrder, path: string[], amountExternal: BigNumber, amountToFill: BigNumber, limitOrderReceiver: string, to: string, keepTokenIn = false) {
        ...
        this.limitOrderReceiverData = utils.defaultAbiCoder.encode(
            ["address[]", "uint256", "address", "bool"],
            [this.path, this.amountExternal.toString(), this.to, keepTokenIn]
        );
    }

    public fillOrderOpen(signer: ethers.Signer, extra: { forceExecution?: boolean, gasPrice?: BigNumber, nonce?: number, debug?: boolean, open?: boolean }) {
        extra.open = true;
        return this.fillOrder(signer, extra);
    }

    public async fillOrder(signer: ethers.Signer | NonceManager, extra: { debug?: boolean, forceExecution?: boolean, gasPrice?: BigNumber, open?: boolean, nonce?: number })
        ...
        const limitOrderContract = new Contract(STOP_LIMIT_ORDER[this.order.chainId], abi, signer);
        ...
        try {
            gasLimit = await limitOrderContract.estimateGas[func](orderArg,
                this.path[0],
        ...
        const transaction: Transaction = await limitOrderContract.fillOrder(
            ...
            this.path[this.path.length - 1],
            this.limitOrderReceiver,
            this.limitOrderReceiverData, { gasLimit, gasPrice, nonce }
        );
